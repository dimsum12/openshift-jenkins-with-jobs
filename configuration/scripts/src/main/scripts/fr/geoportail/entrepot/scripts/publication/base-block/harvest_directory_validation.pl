...
#
# USAGE :
#   This script take a directory and launch a crawl depending on the provider in validation zone
# ARGS :
...
# KEYWORDS
#   $Revision 1 $
#   $Source src/main/scripts/fr/geoportail/entrepot/scripts/publication/harvest_directory_validation.pl $
#   $Date: 30/08/11 $
...
my $logger = Logger->new( "harvest_directory_validation.pl", $logger_levels );

my $url_proxy = $config->param("proxy.url");
my $provider_inspire =
  $config->param("publication.provider.inspire.validation");
my $provider_isoap = $config->param("publication.provider.isoap.validation");
...
sub harvest_directory_validation {
