...
my $logger = Logger->new( "bbox_from_images_standalone.pl", $logger_levels );
...
sub bbox_from_images_standalone {
...
# Listage via l'entête des fichiers images contenus dans le repertoire spécifié
    ...
    foreach my $current_file (@list_files) {
        ...
        my $cmd_file = "file -Li " . $current_file;
        $logger->log( "DEBUG", "Execution de : " . $cmd_file );
        my $return_file   = Execute->run($cmd_file);
        my @file_and_type = $return_file->get_log();

        my @tab_file_and_type = split /: /, $file_and_type[0];
        my $file              = $tab_file_and_type[0];
        my $type              = $tab_file_and_type[1];

        if ( $type =~ /$type_pattern_image/ ) {
            $logger->log( "DEBUG",
                    "\"" 
                  . $type
                  . "\" contient \""
...
"La projection n'est pas un SRID et est introuvable dans le fichier de projection : "
	...
	# Writing the bbox into a "bbox_wkt.txt" file.
	open(BBOXFILE,">".$source_dir."/../".$bboxFileName) or die ("Erreur d'ouverture en écriture du fichier $bboxFileName");
		print BBOXFILE $bbox_wkt;
	close(BBOXFILE);
